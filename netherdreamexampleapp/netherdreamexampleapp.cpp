// netherdreamexampleapp.cpp : Defines the entry point for the console application.
//
...
void DrawSquare(int inDacNumber)
{
	...
	while(true)
	{
		netherdream::Status status = netherdream::GetStatus(inDacNumber);
		if(status == netherdream::Status::Busy)
			continue;
		...
		bool write = netherdream::WriteFrame(inDacNumber, square, size, 30000, 1);
...
void DrawCircle(int inDacNumber, int inShape)
{
	...
	while (true) 
	{
		...
		// Write circle
		bool write = netherdream::WriteFrame(inDacNumber, circle, size, 30000, 1);
		i++;
	}
}
...
void DrawDot(int inDacNumber)
{
	...
	while(true)
	{
		...
		// Write to ether dac
		bool write = netherdream::WriteFrame(inDacNumber, dot, size, 30000, 1);
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	if(!netherdream::LoadEtherDreamLib())
		return Exit("Unable to load ether dream lib!", -1);

	// Successfully loaded and sampled dll
	std::cout << "Successfully loaded EtherDream library\n";

	// Get the card number
	int no_cards = netherdream::GetNumberOfDevices();
	if(no_cards == 0)
	...
	// Get name
	char buffer[256];
	std::string name = netherdream::GetDeviceName(number);
	std::cout << "DAC Name: " << name << "\n";

	// Get status
	netherdream::Status status = netherdream::GetStatus(number);
	std::cout << "DAC Status: " << (int)status << "\n";

	// Open Device
	if(!netherdream::OpenDevice(number))
	...
	// Close device
	bool success = netherdream::Stop(number);

	// Stop
	success = netherdream::CloseDevice(number);

	// Exit
	success = netherdream::Close();
